...
class BaseCrud(Generic[Entity]):
    ...
    def _parse_keys(
        ...
        Принимаемые форматы данных:
            ...
            * кортеж из вышеперечисленных вариантов (если указан флаг __allow_multiple_keys__), например
    ...
    def _apply_default_filtering(self, query: Select, **filter_params: Any) -> Select:
        """
        Вариант фильтрации "по умолчанию", когда не указаны фильтра для CRUD модели
        (с фильтрацией по параметрам модели).
