...
class BaseModelClass:
    ...
    def __repr__(self):
        ...
        local_attrs = [
            i
            for i in class_mapper(self.__class__).iterate_properties
            if i.key not in self.__hidden_fields__
        ]
        local_attrs.sort(key=lambda x: order.index(x.key))
        attrs_repr = [
            f"{i.key}={getattr(self, i.key, None)}"
            for i in local_attrs
            if isinstance(i, sqlalchemy.orm.ColumnProperty)
        ]
...
def fresh_timestamp() -> Callable[[], datetime]:
    """
